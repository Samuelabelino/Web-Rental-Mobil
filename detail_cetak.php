...
$kode = $_GET['kode'];
$sql1 	= "SELECT booking.*,rumah.*, merek.*, users.* FROM booking,rumah,merek,users WHERE booking.id_rumah=rumah.id_rumah 
			AND merek.id_merek=rumah.id_merek and booking.email=users.email and booking.kode_booking='$kode'";
$query1 = mysqli_query($koneksidb, $sql1);
$result = mysqli_fetch_array($query1);
$harga	= $result['harga'];
$durasi = $result['durasi'];
$totalrumah = $durasi * $harga;
$penyewarcharges = $result['penyewa'];
...
<head>
	...
	<meta name="description" content="rental rumah">
		...
		<div class="container-fluid">
			...
			<table class="table table-borderless">
				<tbody>
					...
					<tr>
						<td>Biaya rumah (<?php echo $result['durasi']; ?>) Hari</td>
						<td>:</td>
						<td><?php echo format_rupiah($totalrumah); ?></td>
					</tr>
					<tr>
						<td>Biaya penyewa (<?php echo $result['durasi']; ?>) Hari</td>
						<td>:</td>
						<td><?php echo format_rupiah($penyewacharges); ?></td>
					</tr>
					<tr>
